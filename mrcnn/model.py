...
class MetricsCallback(Callback):
    def __init__(self, train_model, inference_model, dataset,
        ...
        # self.file_writer = tf.summary.FileWriter(self.inference_model.logdir)
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch > 0 and epoch % self.calculate_at_every_X_epoch == 0:
            ...
            mars, mAPs, precisions, recalls = self._calculate_mean_average_precision(epoch)

            mars = np.nanmean(mars)
            mAP = np.nanmean(mAPs)
    ...
    def _calculate_mean_average_precision(self, epoch):
        ...
        for idx, image_id in tqdm(enumerate(self.dataset_image_ids[:self.dataset_limit])):
            image, image_meta, gt_class_id, gt_bbox, gt_mask = load_image_gt(self.dataset,
            ...
            #     image = tf.summary.image("Training data", masked_image)
            #     self.file_writer.add_summary(image).eval()

            # Compute mAP - VOC uses IoU 0.5
            avg_prec, prec, rec, _ = utils.compute_ap(gt_bbox, gt_class_id, gt_mask, r["rois"],
                                                      r["class_ids"], r["scores"], r['masks'])

            avg_recall = compute_ar(r['rois'], gt_bbox)
